...
namespace WPFGedcomParser.Types
{
    public class GEDCOMParser : IGEDCOMParser
    {
                        ...
                        if (mainTag.childrenTags == 0)
                        {
                            ...
                            if (values.Length == 3)
                            {
                                try
                                {
                                    ...
                                    switch (mainTag.Tag)
                                    {
                                        case GEDCOMTags.BIRT:
                                            newIndividual.Birth.Date.Dates.Add(date);
                                            break;
                                        case GEDCOMTags.MARR:
                                            newMarriage.Date.Dates.Add(date);
                                            break;
                                        case GEDCOMTags.DEAT:
                                            newIndividual.Death.Date.Dates.Add(date);
                                            break;
                                    }
                                }
                                catch (Exception)
                    ...
                    else if (tag == GEDCOMTags.PLAC.ToString())
                    {
                        if (mainTag.childrenTags == 0)
                        {
                            ...
                            if (values.Length == 3)
                            {
                                ...
                                {
                                    case GEDCOMTags.BIRT:
                                        newIndividual.Birth.Place = place;
                                        break;
                                    case GEDCOMTags.MARR:
                                        newMarriage.Place = place;
                                        break;
                                    case GEDCOMTags.DEAT:
                                        newIndividual.Death.Place = place;
                                        break;
                                }
                            }
                        }
        ...
        public void GenerateTree(TreeViewItem treeViewItem, Marriage marriage)
        {
            ...
            var newTreeViewItem = new TreeViewItem { Header = string.Format("[{0}] ({1} + {2})", familyName.ToUpper(), husbandName, wifeName) };
                ...
                if (Individuals[child].MarriageIDs.Count > 0)
                    foreach (int m in Individuals[child].MarriageIDs)
                    {
                        ...
                        if (Marriages.TryGetValue(m, out match))
                            GenerateTree(treeViewItem, match);
                    }
                else
                    treeViewItem.Items.Add(new TreeViewItem { Header = Individuals[child].GetNames() });
            }
        }
